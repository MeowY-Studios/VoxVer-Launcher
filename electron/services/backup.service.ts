...
export async function createBackup(
  ...
  try {
    ...
    // 文件名：voxver-backup-YYYYMMDD-HHMMSS.voxverbackup
    const fileName = `voxver-backup-${new Date()
      .toISOString()
      .replace(/[:T]/g, '-')
      .slice(0, 19)}.voxverbackup`
...
export async function restoreBackup(
  ...
  try {
    ...
    if (!manifestEntry)
      return { ok: false, error: '不是有效的 VoxVer Launcher 备份文件（缺少 manifest.json）' }
...
export function listBackups(): BackupListItem[] {
  ...
  for (const f of files) {
    ...
    try {
      ...
      if (stat.isFile() && (f.endsWith('.voxverbackup') || f.endsWith('.zip'))) {
        result.push({
          fileName: f,
          filePath: full,
          size: stat.size,
