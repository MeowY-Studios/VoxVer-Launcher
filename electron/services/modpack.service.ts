...
export async function packAsMrpack(
  ...
  try {
    ...
    const name = options.name || 'VoxVer-Modpack'
    ...
    const manifest: ModpackManifest = {
      ...
      versionId: version,
      name,
      summary: options.summary || `由 VoxVer Launcher 自动生成的整合包（${options.mcVersion}）`,
      files: [],
      dependencies: {
...
export function getDefaultModpackOutputDir(): string {
  const dir = path.join(app.getPath('downloads'), 'VoxVer-Modpacks')
