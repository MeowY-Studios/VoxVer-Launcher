  ...
  <div class="account-card" :class="{ active: account.isActive === 1, [account.type]: true }">
    ...
    <div class="card-avatar">
      ...
      <span class="type-tag" :class="account.type">
        {{ account.type === 'microsoft' ? $t('component.genuine') : $t('component.offline') }}
      </span>
    </div>
    ...
    <div class="card-actions" v-if="showActions">
      <button
        ...
        :title="$t('component.setActive')"
      >
        {{ $t('component.switchAccount') }}
      </button>
      <button class="action-btn danger" @click="$emit('delete', account.id)" :title="$t('common.delete')">
        {{ $t('component.deleteAccount') }}
      </button>
    </div>
  </div>
</template>
