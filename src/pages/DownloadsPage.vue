<template>
  <div class="downloads-page" ref="pageRef">
    ...
    <template v-if="activeCategory === 'vanilla'">
      ...
      <div class="dl-header">
        ...
        <div class="dl-header-text">
          <h3 class="dl-title">{{ $t('download.vanilla') }}</h3>
          <p class="dl-subtitle">{{ $t('download.vanillaSubtitle') }}</p>
        </div>
      </div>
      ...
      <div class="dl-search">
        ...
        <input type="text" v-model="verSearch" :placeholder="$t('download.searchVersion')" />
        <button class="btn-refresh" @click="refreshVersions" :title="$t('download.refresh')">
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
        ...
        <div class="acc-group vox-card">
          ...
          <div class="acc-body" v-show="accOpen.release">
            ...
            >
              ...
              <div class="ver-info">
                <div class="ver-header">
                  ...
                  <span class="ver-type-tag release">{{ $t('download.release') }}</span>
                </div>
                <span class="ver-date">{{ ver.date }}</span>
                <p class="ver-desc">{{ ver.desc }}</p>
              </div>
              <div class="ver-actions" :class="{ visible: ver.hovered }">
                <button class="va-btn" @click.stop="goToDetail(ver)" :title="$t('download.saveAs')">
                  <svg
                    width="13"
                    height="13"
                    viewBox="0 0 24 24"
                ...
                <button class="va-btn" @click.stop="downloadServer(ver)" :title="$t('download.downloadServer')">
                  <svg
                    width="13"
                    height="13"
                    viewBox="0 0 24 24"
            ...
            <div v-if="filteredBySearch(releaseVersions).length === 0" class="acc-empty">
              {{ isLoadingVersions ? $t('download.loading') : $t('download.noResults') }}
            </div>
          </div>
        </div>
          ...
          <div class="acc-body" v-show="accOpen.snapshot">
            ...
            >
              ...
              <div class="ver-info">
                <div class="ver-header">
                  <span class="ver-name">{{ ver.name }}</span>
                  <span class="ver-type-tag snapshot">{{ $t('download.snapshot') }}</span>
                </div>
                <span class="ver-date">{{ ver.date }}</span>
                <p class="ver-desc">{{ ver.desc }}</p>
              </div>
          ...
          <div class="acc-body" v-show="accOpen.old">
            ...
            >
              ...
              <div class="ver-info">
                <div class="ver-header">
                  <span class="ver-name">{{ ver.name }}</span>
                  <span class="ver-type-tag old">{{ $t('download.oldAlpha') }}</span>
                </div>
                <span class="ver-date">{{ ver.date }}</span>
                <p class="ver-desc">{{ ver.desc }}</p>
              </div>
          ...
          <div class="acc-body" v-show="accOpen.april">
            ...
            >
              ...
              <div class="ver-info">
                <div class="ver-header">
                  <span class="ver-name">{{ ver.name }}</span>
                  <span class="ver-type-tag april">{{ $t('download.aprilVersion') }}</span>
                </div>
                <span class="ver-date">{{ ver.date }}</span>
                <p class="ver-desc">{{ ver.desc }}</p>
              </div>
    ...
    <template v-else>
      ...
      <section class="filter-section vox-card">
        ...
        <div class="filter-grid">
          <div class="f-row">
            ...
            <input
              ...
              v-model="searchName"
              :placeholder="$t('download.enterKeyword')"
            />
          </div>
          <div class="f-row">
            <label>{{ $t('download.source') }}</label>
        ...
        <div class="filter-actions">
          <button class="btn-search" @click="doSearch">{{ $t('download.search') }}</button>
          <button class="btn-reset" @click="resetSearch">{{ $t('download.resetFilters') }}</button>
          <button v-if="activeCategory === 'modpack'" class="btn-secondary">{{ $t('download.installExistingModpack') }}</button>
        </div>
      </section>
    ...
    <!-- 回到顶部 -->
    <button v-show="showBackToTop" class="btn-back-to-top" @click="scrollToTop" :title="$t('download.backToTop')">
      <svg
        width="18"
        height="18"
        viewBox="0 0 24 24"
